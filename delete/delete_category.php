...
try {
    ...
    $query = $writeDB->prepare("DELETE FROM category_" . DB::$AppName . " WHERE id = :id ");
  ...
  $image = $query->fetch();
    ...
    $response->addMessage('Deleted Category');
    ...
    $response->setHttpStatusCode(500);
    $response->setSuccess(false);
    $response->addMessage('There was an issue Delete Category - please try again' . $ex);
    $response->send();
    exit;
}
